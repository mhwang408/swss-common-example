...
script_log="/tmp/swss_vlan_script_${run_id}.log"
...
run_runner src/vlan_table/config_vlan_command.py add "$vlan_id" --log-file "$script_log"
...
run_runner src/vlan_table/vlanmgrd.py --vlan-id "$vlan_id" --log-file "$script_log"
...
run_runner src/vlan_table/vlanorch.py --vlan-id "$vlan_id" --log-file "$script_log"
...
run_runner src/vlan_table/syncd.py --vlan-id "$vlan_id" --log-file "$script_log"
...
show_cmd "Key Redis MONITOR operations"
grep -E \
    "__VERIFY_MARKER|VLAN\\|${vlan_key}|VLAN_TABLE_KEY_SET|_VLAN_TABLE:${vlan_key}|VLAN_TABLE:${vlan_key}|ASIC_STATE:SAI_OBJECT_TYPE_VLAN" \
    "$monitor_log" | sed 's/^/  /'
...
cat <<EOF
  ...
  Script DB snapshot log: $script_log
EOF
